...
export default class BasketPage extends React.Component {
  ...
  render() {
    const clothesWithLocalization = mapClothesWithLocalization(
      this.props.basket ? this.props.basket.dirtyClothes : []);
      ...
      <div className="text-center">
        <Helmet title="Корзина для стирки"
                meta={[
                  {
                    "name": "description",
                    "content": "Персональный помощник в стирке - корзина"
                  },
            ...
            clothesWithLocalization.map(item => {
              return (
                <ListItem key={item.id} className="clothes-list">
                  <ListItemAvatar>
                    <Avatar>
                      <img src={process.env.API_URL + "/resource" + item.imgSrc}
